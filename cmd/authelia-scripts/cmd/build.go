...
func buildAutheliaBinaryGOX(xflags []string) {
	...
	go func() {
		...
		cmd.Env = append(os.Environ(),
			"GOEXPERIMENT=nosynchashtriemap,nospinbitmutex", "CGO_CPPFLAGS=-D_FORTIFY_SOURCE=2 -fstack-protector-strong", "CGO_LDFLAGS=-Wl,-z,relro,-z,now",
	...
	go func() {
		...
		cmd := utils.CommandWithStdout("bash", "-c", "docker run --rm -e GOEXPERIMENT=nosynchashtriemap,nospinbitmutex -e GOX_LINUX_ARM_CC=arm-linux-gnueabihf-gcc -e GOX_LINUX_ARM64_CC=aarch64-linux-gnu-gcc -e GOX_FREEBSD_AMD64_CC=x86_64-pc-freebsd14-gcc -v ${PWD}:/workdir -v /buildkite/.go:/root/go authelia/crossbuild "+
